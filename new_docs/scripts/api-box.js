...
var html = fs.readFileSync(
    path.join(__dirname, "..", "assets", "api-box.html"),
    "utf8"
)
...
if (!hexo.config.api_box || !hexo.config.api_box.data_file) {
    throw new Error(
        "You need to provide the location of the api box data file in config.api_box.data_file"
    )
}
...
signature = function(data, options) {
    ...
    if (data.istemplate) {
        ...
        if (data.scope === "instance") {
            ...
            var memberOfData =
                apiData(data.memberof) ||
                apiData(`${data.memberof}#${data.memberof}`)
